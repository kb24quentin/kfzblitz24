...
export async function updateContactAction(formData: FormData) {
  ...
  // Email-uniqueness enforced by DB, aber vor dem update prüfen für bessere
  // fehlermeldung — sonst kommt hässlicher Prisma-P2002-error hoch.
  const existing = await prisma.contact.findFirst({
    where: { email, NOT: { id: contactId } },
    select: { id: true },
  });
  if (existing) {
    throw new Error(`Ein anderer Kontakt hat bereits die E-Mail ${email}`);
  }
